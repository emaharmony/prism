...
func TestE2E_AutoGeneratedIDs(t *testing.T) {
	...
	resolved := cfg.ResolveAgentIDs()
	if resolved.Agents[0].ID != "prism1" {
		t.Errorf("Expected first auto-ID 'prism1', got %q", resolved.Agents[0].ID)
	}
	if resolved.Agents[1].ID != "prism2" {
		t.Errorf("Expected second auto-ID 'prism2', got %q", resolved.Agents[1].ID)
	}

	if err := resolved.Validate(); err != nil {
		t.Fatalf("Auto-generated config validation failed: %v", err)
	}
}
