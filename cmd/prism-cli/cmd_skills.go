// Package main implements the `prism skills` subcommand: discover and inspect the
// SKILL.md skills (Claude Code / OpenClaw) available to agents.
//
// Usage:
//
//	prism skills [--root .] [--json]      List discovered skills
//	prism skills show <name> [--root .]   Print a skill's full instructions
//
// It scans the conventional skill dirs (.claude/skills, .openclaw/skills, skills/)
// under --root, the same discovery `prism serve` uses, so what you see here is what
...
func renderSkillList(infos []skill.Info) string {
	...
	b.WriteString(strings.Repeat("─", 60) + "\n")
	fmt.Fprintf(&b, "%d skill(s). Agents invoke one with the use_skill tool. `prism skills show <name>` for details.\n", len(infos))
	return b.String()
}

// executeSkills is the `prism skills` entry point.
...
func skillsShow(name string, args []string) {
	...
	if name == "" {
		fmt.Fprintln(os.Stderr, "❌ usage: prism skills show <name>")
		os.Exit(1)
	}
	reg := skill.NewRegistry()
