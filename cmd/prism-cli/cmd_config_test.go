...
func TestSummarizeConfig(t *testing.T) {
	cfg := &orchestrator.Config{
		Prism: orchestrator.PrismConfig{InstanceID: "prism", WorkflowConfig: "wf.yaml"},
...
func TestRenderConfigSummary(t *testing.T) {
	out := renderConfigSummary(configSummary{
		InstanceID: "prism", Agents: 2, PrimaryAgent: "lead", Channels: 1,
		MCPServers: 1, AutopatchMode: "pr", Remembrance: true, WorkflowFile: "wf.yaml",
	})
	for _, want := range []string{"is valid", "prism", "primary: lead", "mcp servers: 1", "autopatch:   pr", "remembrance: enabled", "wf.yaml"} {
