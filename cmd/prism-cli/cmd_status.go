// Package main implements the `prism status` subcommand.
//
// Usage:
//
//	prism status [--config prism.yaml]
//
// Shows the current state of a running Prism instance:
...
func executeStatus(args []string) {
	...
	configPath := statusCmd.String("config", "prism.yaml", "Path to prism.yaml configuration file")
	...
	fmt.Println("Note: Run 'prism serve' to start the daemon.")
}
