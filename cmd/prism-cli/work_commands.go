...
func (cc *conversationContext) handlePrismCommand(msg *discordbot.InboundMessage) bool {
	...
	case "delegate", "status", "stop", "cancel":
		if cc.crossCoord == nil {
			cc.sendWorkCommandReply(msg.ChannelID, "Cross-Prism commands are not configured in this instance.")
			return true
		}
		return cc.handleCrossPrismCommand(msg)
	default:
		cc.sendWorkCommandReply(msg.ChannelID, "Unknown Prism command. Use `prism work`, `prism delegate`, `prism status`, or `prism stop`.")
	}
	return true
}
...
func (cc *conversationContext) handleWorkStartCommand(msg *discordbot.InboundMessage, command, rest string) {
	...
	if strings.TrimSpace(req.Prompt) == "" {
		cc.sendWorkCommandReply(msg.ChannelID, "Missing task. Example: `prism work project:GudEats task:start a Vite React app`")
		return
	}
...
func (cc *conversationContext) publishWorkflowStart(channelID string, res *workstart.Resolution) {
	...
	if err := cc.natsConn.Publish("prism.workflow.start", payload); err != nil {
...
func (cc *conversationContext) publishWorkflowFeedback(channelID string, payload map[string]any) bool {
	...
	if err := cc.natsConn.Publish("prism.workflow.feedback.response", data); err != nil {
...
func (cc *conversationContext) sendLocationQuestion(channelID string, res *workstart.Resolution) {
	...
	if res.Recommendation != "" {
		...
		b.WriteString("\n\nNo configured write root is available. Configure `prism.write_roots`, then retry with `path:<absolute path>`.")
	}
