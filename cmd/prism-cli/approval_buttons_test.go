...
func TestEncodeDecodeFeedbackButtonID(t *testing.T) {
	...
	if id != "prismfb:pre:approve:gl-123" {
...
func TestDecodeFeedbackButtonIDRejectsForeign(t *testing.T) {
	for _, bad := range []string{"", "other:pre:approve:gl-1", "prismfb:pre:approve", "prismfb::approve:gl-1", "prismfb:pre:approve:"} {
...
func TestFeedbackButtonPayload(t *testing.T) {
	...
	p, ok := feedbackButtonPayload("prismfb:pre:approve:gl-9", "ema")
	...
	p, ok = feedbackButtonPayload("prismfb:post:changes:gl-9", "")
	...
	// pre/reject → rejected
	if p, ok = feedbackButtonPayload("prismfb:pre:reject:gl-9", "x"); !ok || p["decision"] != "rejected" {
...
func TestFeedbackButtonPayloadGuards(t *testing.T) {
	...
	if _, ok := feedbackButtonPayload("prismfb:pre:approve:run-1", "x"); ok {
		t.Fatal("non gl- run id should be rejected")
	}
	// unknown action
	if _, ok := feedbackButtonPayload("prismfb:pre:explode:gl-1", "x"); ok {
