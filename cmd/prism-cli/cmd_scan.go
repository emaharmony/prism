// Package main implements the `prism scan` subcommand: run the autopatch issue
...
// or PR; that is the gated autopatch pipeline's job (prism serve / autopatch mode).
package main
...
func scanStart(configPath, root string, minSev autopatch.IssueSeverity, issues []autopatch.Issue) {
	...
	dataDir := cfg.Prism.DataDir
	if dataDir == "" {
		dataDir = filepath.Join(".prism", "data")
	}
...
// executeScan is the `prism scan` entry point.
func executeScan(args []string) {
	...
	configPath := fs.String("config", "prism.yaml", "Path to prism.yaml (used by --start)")
	fs.Parse(args)
