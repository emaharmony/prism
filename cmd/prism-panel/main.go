// Command prism-panel is a small native desktop window that connects to a running
// Prism instance and shows a friendly ASCII "pet" whose mood tracks the agent —
...
//
// It is a read-only consumer of the same SSE event stream `prism watch` uses
...
// the pure-Go `prism` CLI: the root module's `go build ./...` never compiles it.
//
...
// panel survives `prism serve` restarting. This is the "resilient" behaviour.
