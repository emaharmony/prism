...
| P-009 | 2026-06-07 | High (fixed) | Agent loop — when tool loop fails, generic error messages trigger cascading agent responses (6+ message echo chamber). Fixed with loop detection (suppress error-pattern messages from agents) + better error text ("AI service returned an error" instead of "tried to use a tool"). Root cause: Ollama 502 TLS error. | Fixed |
