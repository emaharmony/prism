...
| P-008 | 2026-06-07 | High (implemented) | Tool overeagerness — agents call read_file/project_overview for simple greetings. Architectural fix: ToolRelevanceGate pipeline stage with heuristic keyword+pattern matching. Three decisions: Include/Exclude/Subset. 15 unit tests. Mango review: 9.0/10, passes. | Implemented |
| P-009 | 2026-06-07 | High (fixed) | Agent loop — when tool loop fails, generic error messages trigger cascading agent responses (6+ message echo chamber). Fixed with loop detection (suppress error-pattern messages from agents) + better error text. | Fixed |
| P-010 | 2026-06-07 | Feature | Per-channel state actions — config-driven prompt injection based on channel role. Each channel (manager-room, build-room, fun) and agent context gets its own behavioral instructions injected into the system prompt. Replaces static AGENTS.md channel routing with YAML config. | Implemented |
