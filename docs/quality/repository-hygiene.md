...
*   **Runtime State:**
    ...
    *   Verified that `prism-data/` and `prism-workspace/` are correctly ignored.
*   **One-off Utilities:**
...
*   **`.gitignore`:** Hardened to ensure no local binaries (`*.exe`) or runtime databases (`*.db`) from the root or `.prism/` are committed.
